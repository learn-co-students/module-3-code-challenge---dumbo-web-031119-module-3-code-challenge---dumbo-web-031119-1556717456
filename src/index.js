...
const listOfBeers = (beer) => {
    return `<ul class="list-group"><li class="list-group-item">${beer.name}</li></ul>`
}
...
fetch('http://localhost:3000/beers')
    ...
    .then(arrayOfBeers => {
        arrayOfBeers.forEach((beer) => {
            beerList.innerHTML += listOfBeers(beer)
        })
    })

// Step 2 - Display Single Beer Details

const beerDetailHTML = (beer) => {
...
beerList.addEventListener('click', (event) => {
    if (event.target.className === 'list-group-item') {
        ...
        .then(res => res.json())
        .then((beer) => {
            beerDetails.innerHTML += beerDetailHTML(beer)
        })
    }
})
